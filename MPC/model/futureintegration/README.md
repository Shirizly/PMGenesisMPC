...
**The model files themselves are no longer duplicated here.** All four were
byte-identical to upstream's own copies, so this directory now holds only the
consolidation notes and points at the originals:

    GranularDynamics2/myClasses/{Diff_Renderer,MultiExitUnet,UNetModels,UNetModels_conditioned}.py

They import from the repo root as `GranularDynamics2.myClasses.<module>`
(an implicit namespace package - there is no `__init__.py` and none is needed).

Two reasons for not copying them. Upstream maintains these files and a
duplicate would silently drift from them; and `Diff_Renderer.py` does not
parse - it has an unfinished `tool_mask = ` at line 33 - so a copy of it here
broke this package's import smoke test for a defect that is not this
package's to fix.

The notes below are unchanged from the 2026-07 cleanup.
