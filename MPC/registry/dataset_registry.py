...
def _build_genesis_dataset(cfg: dict, split: str) -> EulerianDatasetWrapper:
    ...
    """
    import genesis_path  # noqa: F401  (puts Genesis/ on sys.path)
    from training.dataset import PileSweepData

    # `physics.normalization` in a dataset config is read for the model card
    # only; the dataset normalises with fixed bounds.
    #
    # PileSweepData._det_physics maps (friction, density, box_friction) onto
    # [0, 1] over friction 0.05-0.50, density 750-5000, box friction 0.05-0.50.
    # Those are not arbitrary constants - they are the endpoints of the
    # original collection sweep, so the normalisation spans exactly the range
    # the data was drawn from. Passing bounds in was tried and removed: it made
    # three numbers configurable that nothing currently varies, at the cost of
    # a second source of truth for them.
    #
    # If a sweep ever uses a different range those constants become wrong
    # rather than merely fixed. The place to put the override back is the
    # WRAPPER, not the dataset - which is exactly what the real-data path
    # below already does: RealPileSweepData returns raw physical units and
    # _RealEulerianDatasetWrapper applies bounds.normalize(). Copying that
    # pattern here keeps one source of truth for the bounds and leaves
    # upstream's dataset alone. See PORT_NOTES.md section 12.
    raw = PileSweepData(
        paths=cfg["paths"],
        split=split,
