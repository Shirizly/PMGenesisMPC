"""Transition buffer for MPC-side recording.

Moved out of the simulator: this is MPC machinery, not simulator
machinery. Upstream's collection path records through
`collect_data_samples` + `_save_rollout` + the DINO/LeWM exporters, so
putting a second recording mechanism into `Genesis/sandbox_manipulation.py`
would leave two overlapping ones there. It is attached instead by
`MPC/env/recording_sandbox.py`, which subclasses the simulator.

Otherwise unchanged from the historical version.
"""
