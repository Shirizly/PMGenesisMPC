...
- **One loss registry serves both training and MPC cost.** `model_training/losses.py`
...
- **The simulator core stays purpose-agnostic.** `Genesis/sandbox_manipulation.py`
  ...
  (automatic MPC transition recording, `MPC/env/transition_buffer.py`) are the
...
model/
  ...
  futureintegration/    consolidation NOTES ONLY. The four architectures
                        that lived here were byte-identical to upstream's
                        GranularDynamics2/myClasses/ copies and were removed
                        rather than duplicated; import them from there as
                        GranularDynamics2.myClasses.<module>. The three that
                        ARE registered live directly in model/ (below).
...
model_training/          (named to avoid colliding with Genesis/training/)
...
Genesis/
  sandbox_manipulation.py  SandboxManipulation — low-level multi-env
                        ...
                        data_collection.py, not oracle-specific.
  state_library.py       StateLibrary / build_state_library — a bank of
                        *settled* pile states, so a reset is a pose write
                        ...
                        from data_collection.py via --state-library.
  ...
  transition_buffer.py   TransitionBuffer — accumulates and saves the
                        ...
                        data_collection.py's dataset files use.
                        Genesis-free (no `import genesis`), independently
                        unit-tested.
  data_collection.py  batched random-push dataset collection; the
  ...
  (removed — use tests/benchmarks/bench_performance.py)    throughput, for picking simple_mpc.oracle_mpc's
                        n_envs default; run as `tests/benchmarks/bench_performance.py`
...
tests/
  ...
  scaling_investigation/         archived probes, raw measurements and the
                                  ...
                                  Genesis/sandbox_manipulation.py;
...
Entry points beyond `model_training/train.py`:
...
1. Subclass `LossFn` in `model_training/losses.py`.
...
Notes:
- `model_training/trainer.py` resolves top-level string paths ending in `.yaml` and
