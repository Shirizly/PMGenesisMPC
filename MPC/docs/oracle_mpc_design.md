...
`Genesis/data_collection.py`'s multi-env batching):
...
- **All K envs, including env 0, are rollout workers during planning.** Each
  ...
  `Genesis/data_collection.py` uses. Env 0 is not special-cased out of
...
`Genesis/sandbox_manipulation.py`) is the low-level primitive this
...
scored using `model_training.losses.build_loss`, exactly as training does, with
...
batch-reduced scalar — see `model_training/losses.py`'s module docstring and
INTERFACES.md §3.5). `mpc.loss` in config selects the loss type and weights;
any loss registered in `model_training/losses.py` is usable here for free,
...
(`Genesis/sandbox_manipulation.py`) fires `callback('post_lower')` and
...
| `model_training/losses.py` | `EulerianCombinedLoss`'s `per_sample` mode, `ScoreMapWeightedLoss` |
| `Genesis/sandbox_manipulation.py` | `execute_action(..., on_phase=...)`, `set_particle_state`, `broadcast_state_from_env`, `push_and_record`/`flush_transitions` — shared, not oracle-specific |
| `MPC/env/transition_buffer.py` | `TransitionBuffer` — accumulates/saves the transitions `push_and_record` records (see UTILITIES.md §1.2) |
| `tests/benchmarks/bench_performance.py` | Throughput measurement for picking `mpc.n_envs` (replaces the old `Genesis/benchmark_n_envs.py`, which was not ported) |
...
| `n_envs` | Parallel Genesis envs = candidates evaluated per optimizer iteration; pick via `tests/benchmarks/bench_performance.py` |
...
| `loss` | Passed to `model_training.losses.build_loss` (`per_sample` is forced on regardless of what's set here) |
...
```bash
python tests/benchmarks/bench_performance.py                      # pick mpc.n_envs first
...
- **Compute cost.** A single MPC step at `n_envs=256`, `n_opt_iter=5-10` has
  ...
  budget accordingly, and prefer `tests/benchmarks/bench_performance.py` plus small
