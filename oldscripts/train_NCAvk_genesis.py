##############################################################
# THIS SCRIPT IS USED TO TRAIN AN NCA MODEL WITH GENESIS DATA
# Adapted from train_unet_genesis.py with minimal changes.
#
# === Key differences vs. the UNet training script ===
#
# 1. MODEL API
#    NFDUNetFiLM : forward(x, props) -> logit (B,1,H,W)
#    NCAWithPhysics: forward(x, props) -> logit (B,1,H,W)   ← same external API
#    Internally, NCAWithPhysics broadcasts `props` to spatial maps and
#    concatenates them with `x` before passing to NCAPlusUNet.  Physics
#    conditioning therefore enters as extra input channels at every NCA step
#    rather than as FiLM scale/bias on intermediate features.
#
# 2. OUTPUT SEMANTICS — "logit" vs. "direct state"
#    Both models return an unbounded value treated as a logit.  However their
#    dynamics differ at initialisation:
#      UNet: head(d1) + x[:,0:1].  head initialised ~0 → output ≈ x[:,0:1] ∈ [0,1].
#            sigmoid(x[:,0:1]) ∈ [0.5, 0.73] → MSE loss immediately pulls toward {0,1}.
#      NCA:  state + Σ_k delta_k.  deltas initialised ~0 → output ≈ state ∈ [0,1].
#            Same initialisation behaviour as UNet, so the same loss functions work.
#    At convergence, both models are pushed to produce large-magnitude logits
#    (±3…±5) so that sigmoid saturates near {0,1}.
#
# 3. GRADIENT FLOW
#    The NCA runs K sequential local conv steps (default K=8).  Gradients
#    back-propagate through all K steps (BPTT-style).  With the gated update
#    `delta = dt * raw_delta * (1 + tanh_gate)` and dt=0.5, gradient magnitudes
#    stay bounded, and the existing grad-clip (max_norm=1.0) prevents explosions.
#    Empirically, more NCA steps → richer spatial dynamics but longer per-epoch
#    time and higher memory.  Start with K=8 and increase if underfitting.
#
# 4. PHYSICS CONDITIONING QUALITY
#    FiLM (UNet): γ(props) ⊙ features + β(props) at every conv block — learned
#      nonlinear modulation of feature statistics.  Initialised to identity so
#      training starts as unconditioned.
#    Channel broadcast (NCA): physics appears as constant extra channels at
#      every local update step.  Simpler but still gives the NCA access to
#      physics at every spatial location every step.  The NCA can learn to gate
#      updates based on physics values.
#
# 5. MULTI-STEP / MPC ROLLOUT
#    The NCA's iterative structure makes it naturally suited to multi-step
#    rollout:
#      • "Unrolling" more steps at inference increases effective receptive field
#        without retraining.  Plate geometry influence can propagate further.
#      • Variable-step rollout: run K_train steps during training, K_infer > K_train
#        at inference — the NCA shares weights across steps so this extrapolates.
#      • For MPC gradient-based optimisation (see CODEBASE_OVERVIEW §11), sigmoid
#        must be applied to the output before computing the reward, exactly as
#        for the UNet.  A future NCAFiLMPushModel adapter would mirror
#        UNetFiLMPushModel, applying sigmoid and handling the coordinate flip.
#
# 6. TRAINING SPEED
#    K=8 NCA steps + one UNet correction pass per sample.  Expect ~1.5–2× longer
#    forward pass than the UNet.  Reduce nca_steps or unet_features if GPU memory
#    is tight.
##############################################################
...
from GranularDynamics2.myClasses.NCAModels import NCAWithPhysics
...
DEFAULT_LOG_DIR = Path("runs_cubes/nca_mse_mass2")
RESUME_TRAINING = True

# NCA-specific hyperparameters
NCA_HIDDEN = 64       # hidden channels inside each NCA conv block
NCA_STEPS = 8         # number of iterative local update steps (= K_MAX default)
UNET_FEATURES = [16, 32]  # correction UNet encoder channel widths

# Variable-K bucketing hyperparameters
N_BUCKETS = 4   # number of action-length quantile buckets
K_MIN = 2       # NCA steps for the shortest-action bucket
# K_MAX defaults to NCA_STEPS (longest-action bucket uses the full step budget)


def parse_args():
    parser = argparse.ArgumentParser(description="Train or evaluate the Genesis NCA model.")
    parser.add_argument("--eval-only", action="store_true", help="Load a checkpoint and evaluate without training.")
    parser.add_argument(
        "--checkpoint",
        type=Path,
        default=None,
        help="Checkpoint to load for --eval-only. Defaults to <log-dir>/nca.pth.",
    )
    ...
    parser.add_argument("--nca-hidden", type=int, default=NCA_HIDDEN, help="Hidden channels in each NCA conv block.")
    parser.add_argument("--nca-steps", type=int, default=NCA_STEPS, help="Number of NCA iterative update steps.")
    ...
    parser.add_argument("--n-buckets", type=int, default=N_BUCKETS, help="Number of action-length quantile buckets for variable-K sampling.")
    parser.add_argument("--k-min", type=int, default=K_MIN, help="Minimum NCA steps (shortest-action bucket).")
    parser.add_argument("--k-max", type=int, default=None, help="Maximum NCA steps (longest-action bucket). Defaults to --nca-steps.")
...
def latest_epoch_checkpoint(log_dir: Path) -> Path | None:
    checkpoints = []
    for path in log_dir.glob("nca_epoch_*.pth"):
...
def default_resume_checkpoint(log_dir: Path) -> Path | None:
    ...
    if latest is not None:
        return latest
    best = log_dir / "nca_best.pth"
    if best.exists():
        return best
    final = log_dir / "nca.pth"
    if final.exists():
        return final
    return None
...
def _compute_action_lengths(dataset) -> torch.Tensor:
    """
    Compute the Euclidean action length in metres for every sample in the dataset.

    Reads dataset.runs directly (avoids __getitem__ rendering overhead).
    The returned tensor is ordered identically to the dataset's global indices
    because runs are appended in order and _run_lookup tiles them the same way.
    """
    parts = []
    for run in dataset.runs:
        diff = run["p_stops"][:, :2] - run["p_starts"][:, :2]  # (N, 2), metres
        parts.append(torch.norm(diff, dim=1))
    return torch.cat(parts)  # (total_samples,)


class ActionLengthBucketSampler(Sampler):
    """
    Bucketed batch sampler for variable-K NCA training.

    Bins the dataset by action length (Euclidean distance between p_starts and
    p_stops) into ``n_buckets`` equal-population quantile buckets.  All samples
    in a batch come from the same bucket, so the model can be called with a
    per-batch NCA step count K.

    K is linearly interpolated between ``k_min`` (shortest bucket) and ``k_max``
    (longest bucket) based on each bucket's median action length.

    Usage::

        sampler = ActionLengthBucketSampler(dataset, batch_size=32, ...)
        loader  = DataLoader(dataset, batch_sampler=sampler, ...)
        for batch_data, k in zip(loader, sampler.iter_k()):
            logits = model(inputs, physics, steps=k)

    Notes
    -----
    - ``__iter__`` rebuilds the batch list (and reshuffles if ``shuffle=True``)
      at the start of every epoch.
    - ``iter_k()`` is a generator that reads ``self._batches``, which is
      populated by the most-recent ``__iter__`` call.  Always ``zip`` it with
      the loader so the two remain in sync.
    - For val/test use ``shuffle=False``; ``__iter__`` is still called once per
      evaluation pass, keeping ``iter_k()`` synchronised.
    """
    ...
    def __init__(
        ...
        lengths = _compute_action_lengths(dataset)
        n_samples = len(lengths)
        n_buckets = max(1, min(n_buckets, n_samples))

        # Equal-population quantile edges
        edges = torch.quantile(lengths, torch.linspace(0.0, 1.0, n_buckets + 1))
        interior = edges[1:-1].contiguous()
        bucket_ids = torch.bucketize(lengths, interior)  # values in [0, n_buckets-1]

        raw_buckets: list[list[int]] = [[] for _ in range(n_buckets)]
        for idx, bid in enumerate(bucket_ids.tolist()):
            raw_buckets[bid].append(idx)

        # Drop empty buckets (can occur when many samples share identical lengths)
        self._buckets: list[list[int]] = [b for b in raw_buckets if b]

        l_min = lengths.min().item()
        l_max = lengths.max().item()
        l_range = max(l_max - l_min, 1e-8)

        self.bucket_k: list[int] = []
        for b_indices in self._buckets:
            med = lengths[b_indices].median().item()
            t = (med - l_min) / l_range
            k = int(round(k_min + t * (k_max - k_min)))
            self.bucket_k.append(max(k_min, min(k_max, k)))

        # _batches is rebuilt each time __iter__ is called
        self._batches: list[tuple[list[int], int]] = []
    ...
    def _rebuild_batches(self) -> None:
        ...
        if self.shuffle:
            for b in buckets:
                random.shuffle(b)
        for b_indices, k in zip(buckets, self.bucket_k):
            for start in range(0, len(b_indices), self.batch_size):
                chunk = b_indices[start : start + self.batch_size]
                if self.drop_last and len(chunk) < self.batch_size:
                    continue
                self._batches.append((chunk, k))
        if self.shuffle:
    ...
    def iter_k(self):
        """Yield the NCA step count K for each batch in the same order as __iter__."""
        for _, k in self._batches:
            yield k

    def __len__(self) -> int:
        if self.drop_last:
            return sum(len(b) // self.batch_size for b in self._buckets)
        return sum((len(b) + self.batch_size - 1) // self.batch_size for b in self._buckets)


def build_model(nca_hidden: int, nca_steps: int) -> NCAWithPhysics:
    return NCAWithPhysics(
        in_channels=2,
        physics_dim=3,
...
def combined_loss(logits, outputs, inputs, criterion):
    # NOTE: The NCA outputs a raw value treated as a logit; sigmoid converts it
    # to an occupancy probability in (0,1), exactly as for the UNet.  At
    # initialisation the NCA output ≈ input state ∈ [0,1], so sigmoid maps it
    # to roughly [0.5, 0.73] — a non-trivial starting point that still yields
    # non-zero MSE gradients from the first batch.
    probs = torch.sigmoid(logits)
...
def evaluate_model(model, loader, criterion, sampler=None):
    model.eval()
    totals = empty_totals()
    k_iter = iter(sampler.iter_k()) if sampler is not None else None
    with torch.no_grad():
        for inputs_, outputs in loader:
            ...
            k = next(k_iter) if k_iter is not None else None
            logits = model(inputs, physics, steps=k).squeeze(1).float()
...
if __name__ == "__main__":
    ...
    K_MIN = args.k_min
    k_max = args.k_max if args.k_max is not None else args.nca_steps
    ...
    print(f"NCA hidden: {args.nca_hidden}")
    print(f"NCA steps:  {args.nca_steps}")
    ...
    if not args.eval_only:
        run_config = {
            "script": "train_NCA_genesis.py",
            "started_at": datetime.now().isoformat(timespec="seconds"),
            "log_dir": str(log_dir),
            "model": {
                "type": "NCAWithPhysics",
                "nca_hidden": args.nca_hidden,
    ...
    model = build_model(args.nca_hidden, args.nca_steps)

    pos_weight = torch.tensor([POS_WEIGHT], device=DEVICE)
    criterion = torch.nn.BCEWithLogitsLoss(pos_weight=pos_weight)

    if args.eval_only:
        eval_sampler = ActionLengthBucketSampler(
            test_dataset, batch_size=args.batch_size,
            n_buckets=N_BUCKETS, k_min=K_MIN, k_max=k_max, shuffle=False,
        )
        ...
        checkpoint = args.checkpoint if args.checkpoint is not None else log_dir / "nca.pth"
        state_dict = torch.load(checkpoint, map_location=DEVICE)
        model.load_state_dict(state_dict)
        print(f"Evaluating checkpoint: {checkpoint}")
        test_metrics = evaluate_model(model, test_loader, criterion, sampler=eval_sampler)
    ...
    train_sampler = ActionLengthBucketSampler(
        train_dataset, batch_size=batch_size,
        n_buckets=N_BUCKETS, k_min=K_MIN, k_max=k_max, shuffle=True,
    )
    val_sampler = ActionLengthBucketSampler(
        val_dataset, batch_size=batch_size,
        n_buckets=N_BUCKETS, k_min=K_MIN, k_max=k_max, shuffle=False,
    )
    test_sampler = ActionLengthBucketSampler(
        test_dataset, batch_size=batch_size,
        n_buckets=N_BUCKETS, k_min=K_MIN, k_max=k_max, shuffle=False,
    )
    train_loader = DataLoader(train_dataset, batch_sampler=train_sampler, num_workers=args.num_workers, pin_memory=DEVICE == "cuda")
    val_loader = DataLoader(val_dataset, batch_sampler=val_sampler, num_workers=args.num_workers, pin_memory=DEVICE == "cuda")
    test_loader = DataLoader(test_dataset, batch_sampler=test_sampler, num_workers=args.num_workers, pin_memory=DEVICE == "cuda")
    print(f"Variable-K buckets: {list(zip(range(len(train_sampler.bucket_k)), train_sampler.bucket_k))}")
    ...
    if start_epoch > 0:
        resume_val_metrics = evaluate_model(model, val_loader, criterion, sampler=val_sampler)
    ...
    with trange(start_epoch, EPOCHS, desc="Training Epochs") as tbar:
        for epoch in tbar:
            ...
            for (inputs_, outputs), k in zip(train_loader, train_sampler.iter_k()):
                inputs, physics = inputs_
                inputs = inputs.to(DEVICE)
                physics = physics.to(DEVICE)
                outputs = outputs.to(DEVICE)
                ...
                with torch.amp.autocast(device_type=DEVICE, dtype=torch.bfloat16):
                    logits = model(inputs, physics, steps=k).squeeze(1)
            ...
            with torch.no_grad():
                for (inputs_, outputs), k in zip(val_loader, val_sampler.iter_k()):
                    inputs, physics = inputs_
                    inputs = inputs.to(DEVICE)
                    physics = physics.to(DEVICE)
                    outputs = outputs.to(DEVICE)
            ...
            if avg_val_loss < best_val_loss:
                ...
                best_epoch = epoch + 1
                epochs_without_improvement = 0
                torch.save(model.state_dict(), log_dir / "nca_best.pth")
            else:
            ...
            if (epoch + 1) % 10 == 0:
                save_path = log_dir / f"nca_epoch_{epoch + 1}.pth"
                torch.save(model.state_dict(), save_path)

            scheduler.step()

        writer.close()
        save_path = log_dir / "nca.pth"
        torch.save(model.state_dict(), save_path)
        ...
        model.eval()
        test_metrics = evaluate_model(model, test_loader, criterion, sampler=test_sampler)
        print_test_metrics(test_metrics)
